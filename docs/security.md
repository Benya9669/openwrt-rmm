...
- The stable agent version is accepted from the remote update manifest only after an
  ECDSA signature verifies against the public package key bundled with the server image.
  The dashboard performs Semantic Version comparison and does not recommend downgrades.
...
- Add automated dependency/container vulnerability scanning and policy checks for the
  SBOM already generated by CI.
- Encrypt webhook secrets and pending notification destinations at the application layer.
